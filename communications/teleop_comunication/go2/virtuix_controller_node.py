...
class VirtuixController(Node):
    def __init__(self, non_control=False):
        ...
        self.linear_sub = self.create_subscription(
            Twist, "/virtuix_linear", self.linear_callback, 10
        )
        self.heading_sub = self.create_subscription(
            Float64, "/virtuix_heading", self.heading_callback, 10
        )
        ...
        if self.non_control:
            self.preset_angular_speed = (
                1.0  # Preset angular speed (radians per second).
            )
    ...
    def heading_callback(self, msg):
        ...
        self.get_logger().info(
            f"Received target heading: {self.target_heading:.2f} rad."
        )
    ...
    def control_loop(self):
        ...
        if self.target_heading is not None and (self.prevTarget is None or self.target_heading != self.prevTarget):
            # Compute the error between target and current yaw.
            error = self.target_heading - self.current_yaw
            error = math.atan2(
                math.sin(error), math.cos(error)
            )  # Normalize to [-pi, pi]
            self.get_logger().info(f"Current angle: {self.current_yaw}, Target angle: {self.target_heading}")
            if self.non_control:
                ...
                if abs(error) > self.yaw_threshold:
                    ...
                    self.prevTarget = self.target_heading
